...
class IndexAddDependencyRepoDialog(project: Project) : DialogWrapper(project) {
    ...
    fun getRemoteRange(): String? = remoteField.text
}
...
class IndexAddDependencyAction : AnAction(
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val dialog = IndexAddDependencyRepoDialog(project)
        if (dialog.showAndGet()) {
            val repo = dialog.getRepo()
            val description = dialog.getDescription()
            val remote = dialog.getRemote()
            val range = dialog.getRemoteRange();
            CoroutineScope(Dispatchers.Default).launch {
                IndexCommands.indexAddDependency(remote, repo, description, range,project)
            }
        }
    }
}